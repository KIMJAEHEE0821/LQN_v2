...
# # Function to draw an EPM bipartite graph
# def Draw_EPM_bipartite_graph(B):
...
#     pos = {}
#     pos.update((node, (0, -index)) for index, node in enumerate(system_nodes))  # Left group 1
#     pos.update((node, (1, -index)) for index, node in enumerate(sculpting_nodes[:num_system]))  # Right group 1
#     pos.update((node, (0, -(index + len(system_nodes)))) for index, node in enumerate(ancilla_nodes))  # Left group 2
#     pos.update((node, (1, -(index + len(system_nodes)))) for index, node in enumerate(sculpting_nodes[num_system:]))  # Right group 2
...
#     # Map edge weights to specific colors for visualization
#     edge_colors_B = ['red' if B[u][v]['weight'] == 1 else 
#                      'blue' if B[u][v]['weight'] == 2 else 
#                      'black' for u, v in B.edges]
...
def EPM_digraph_from_EPM_bipartite_graph(B):
    ...
    adj_matrix_B = nx.to_numpy_array(B, nodelist=sorted(B.nodes))
    adj_weight_matrix_B = nx.to_numpy_array(B, nodelist=sorted(B.nodes), weight='weight')
...
#     for bipartite_graph in EPM_bipartite_graph_generator(num_system, num_ancilla):
...
def graph_generation_test(num_system, num_ancilla):
    bigraph_list_2 = []
    digraph_list_2 = []
    ...
    for bipartite_graph in EPM_bipartite_graph_generator(num_system, num_ancilla):
        ...
        is_unique = True
        for existing_bigraph in unique_bigraphs:
            if nx.is_isomorphic(bipartite_graph[0], existing_bigraph):
                is_unique = False
                break
        if is_unique:
            unique_bigraphs.add(bipartite_graph[0])
            digraph, adj_matrix_D, adj_weight_matrix_D = EPM_digraph_from_EPM_bipartite_graph(
            bipartite_graph[0],
            adj_matrix_B=bipartite_graph[1],
            adj_weight_matrix_B=bipartite_graph[2]
